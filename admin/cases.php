...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fa === 'save') {
        ...
        if ($editId) {
            ...
            if ($lawyerId) {
                create_notification($pdo, (int)$lawyerId, 'notify.case_assigned', notify_payload('notify.msg.case_assigned', ['number' => $caseNumber]), 'case', '../lawyer/cases.php?id=' . $caseId, current_user()['id']);
            }
            create_notification($pdo, $clientId, 'notify.case_opened', notify_payload('notify.msg.case_opened', ['number' => $caseNumber]), 'case', '../client/cases.php', current_user()['id']);
        }
    ...
    if ($fa === 'reopen') {
        ...
        flash('success', __('flash.case.reopened'));
...
if ($action === 'create' || ($action === 'edit' && $id)) {
    ...
    ?>
    <div class="panel">
        <h2><?= $id ? __e('cases.edit') : __e('cases.create') ?></h2>
        <form method="post" class="form-grid">
            <?= csrf_field() ?><input type="hidden" name="form_action" value="save"><input type="hidden" name="id" value="<?= (int)$case['id'] ?>">
            <div class="form-group full"><label><?= __e('common.title') ?></label><input name="title" required value="<?= e($case['title']) ?>"></div>
            <div class="form-group"><label><?= __e('common.type') ?></label><input name="case_type" value="<?= e($case['case_type']) ?>"></div>
            ...
            <div class="form-group"><label><?= __e('common.priority') ?></label>
                <select name="priority"><?php foreach (['low','medium','high','urgent'] as $p): ?><option value="<?= $p ?>" <?= $case['priority']===$p?'selected':'' ?>><?= e(translate_status($p)) ?></option><?php endforeach; ?></select>
            </div>
            <div class="form-group"><label><?= __e('common.client') ?></label>
                <select name="client_id" required><option value=""><?= __e('form.select') ?></option><?php foreach ($clients as $c): ?><option value="<?= (int)$c['id'] ?>" <?= (int)$case['client_id']===(int)$c['id']?'selected':'' ?>><?= e(full_name($c)) ?></option><?php endforeach; ?></select>
            </div>
            <div class="form-group"><label><?= __e('common.lawyer') ?></label>
                <select name="lawyer_id"><option value=""><?= __e('common.unassigned') ?></option><?php foreach ($lawyers as $l): ?><option value="<?= (int)$l['id'] ?>" <?= (int)$case['lawyer_id']===(int)$l['id']?'selected':'' ?>><?= e(full_name($l)) ?></option><?php endforeach; ?></select>
            </div>
            <div class="form-group"><label><?= __e('form.court_name') ?></label><input name="court_name" value="<?= e($case['court_name']) ?>"></div>
            <div class="form-group"><label><?= __e('form.court_location') ?></label><input name="court_location" value="<?= e($case['court_location']) ?>"></div>
            <div class="form-group"><label><?= __e('form.filing_date') ?></label><input type="date" name="filing_date" value="<?= e($case['filing_date']) ?>"></div>
            <div class="form-group"><label><?= __e('form.next_hearing') ?></label><input type="date" name="next_hearing_date" value="<?= e($case['next_hearing_date']) ?>"></div>
            <div class="form-group full"><label><?= __e('common.description') ?></label><textarea name="description"><?= e($case['description']) ?></textarea></div>
            <div class="form-actions full"><button class="btn btn-primary" type="submit"><?= __e('cases.save') ?></button><a class="btn btn-ghost" href="cases.php"><?= __e('common.cancel') ?></a></div>
        </form>
    </div>
...
if ($action === 'view' && $id) {
    ...
    <div class="panel">
        <div class="panel-header">
            <div>
                ...
                <p class="muted"><?= __e('cases.client_label') ?> <?= e($case['client_name']) ?> · <?= __e('cases.lawyer_label') ?> <?= e($case['lawyer_name'] ?: __('common.unassigned')) ?></p>
            </div>
            <div class="quick-links">
                ...
                <form method="post"><?= csrf_field() ?><input type="hidden" name="form_action" value="reopen"><input type="hidden" name="id" value="<?= $id ?>"><button class="btn btn-sm btn-accent" type="submit"><?= __e('cases.reopen') ?></button></form>
                <?php endif; ?>
            </div>
        </div>
        <p><?= nl2br(e($case['description'] ? t_content($case['description']) : __('common.em_dash'))) ?></p>
        <div class="grid grid-3" style="margin-top:1rem;">
            <div class="list-item"><strong><?= __e('common.court') ?></strong><?= e($case['court_name'] ? t_content($case['court_name']) : __('common.em_dash')) ?></div>
            <div class="list-item"><strong><?= __e('common.location') ?></strong><?= e($case['court_location'] ? t_content($case['court_location']) : __('common.em_dash')) ?></div>
            <div class="list-item"><strong><?= __e('form.next_hearing') ?></strong><?= e(format_date($case['next_hearing_date'])) ?></div>
        </div>
    </div>
    <div class="grid grid-2">
        <div class="panel">
            <h2><?= __e('cases.history_notes') ?></h2>
            <form method="post" class="form-grid" style="margin-bottom:1rem;">
                ...
                <div class="form-group full"><textarea name="note" required placeholder="<?= __e('cases.add_note_ph') ?>"></textarea></div>
                <div class="form-group"><label><input type="checkbox" name="is_private" value="1"> <?= __e('cases.private_note') ?></label></div>
                <div class="form-group"><button class="btn btn-sm btn-primary" type="submit"><?= __e('cases.add_note') ?></button></div>
            </form>
            <div class="list-stack">
                <?php foreach ($notes as $n): ?>
                    <div class="list-item"><strong><?= e($n['author']) ?><?= $n['is_private'] ? ' (' . __('lawyer.cases.private') . ')' : '' ?></strong><span class="muted"><?= e(format_datetime($n['created_at'])) ?></span><div><?= nl2br(e(t_content($n['note']))) ?></div></div>
                <?php endforeach; ?>
            </div>
        </div>
        <div class="panel">
            ...
            <form method="post" enctype="multipart/form-data" class="form-grid" style="margin-bottom:1rem;">
                ...
                <div class="form-group full"><button class="btn btn-sm btn-accent" type="submit"><?= __e('common.upload') ?></button></div>
            </form>
            <div class="list-stack">
                <?php foreach ($docs as $d): ?>
                    <div class="list-item"><strong><?= e(t_content($d['title'])) ?></strong><span class="muted"><?= e(__('doc.category.' . $d['category'])) ?> · <?= e(format_datetime($d['created_at'])) ?></span><div><a href="../<?= e($d['file_path']) ?>" target="_blank"><?= __e('common.download') ?></a></div></div>
                <?php endforeach; ?>
    ...
    <div class="panel">
        <div class="panel-header"><h2><?= __e('cases.court_progress') ?></h2><a href="court.php?action=create&case_id=<?= $id ?>"><?= __e('cases.add_hearing') ?></a></div>
            ...
            <table>
                ...
                <?php foreach ($hearings as $h): ?>
                    <tr>
                        ...
                        <td><?= e($h['hearing_type'] ? t_content($h['hearing_type']) : __('common.em_dash')) ?></td>
                        <td><?= status_badge($h['status']) ?></td>
                        <td><?= e($h['outcome'] ? t_content($h['outcome']) : __('common.em_dash')) ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
...
?>
<div class="panel">
    <div class="panel-header"><h2><?= __e('cases.all') ?></h2><a class="btn btn-primary btn-sm" href="?action=create"><?= __e('cases.create') ?></a></div>
    <div class="table-wrap">
        <table>
            <thead><tr><th><?= __e('common.case') ?></th><th><?= __e('common.client') ?></th><th><?= __e('common.lawyer') ?></th><th><?= __e('common.status') ?></th><th><?= __e('common.hearing') ?></th></tr></thead>
            <tbody>
            <?php foreach ($cases as $c): ?>
                <tr>
                    <td><a href="?action=view&id=<?= (int)$c['id'] ?>"><strong><?= e($c['case_number']) ?></strong></a><div class="muted"><?= e(t_content($c['title'])) ?></div></td>
